...
import { Team, Standing, Match, LeagueState, GameView, TransferOffer, FormResult, Difficulty } from './types';
...
const App: React.FC = () => {
  ...
  const handleAdvance = () => {
    ...
    if (leagueState && leagueState.currentWeek >= leagueState.matches.length - 1) {
        ...
        if (sorted[0].teamId === leagueState.playerTeamId) {
    ...
    <div className="w-full h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center overflow-hidden">
          ...
          <div className="flex flex-col gap-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
              <div className="space-y-4">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Match Time</label>
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Target Score</label>
                <div className="flex flex-wrap justify-center gap-2">
                  {[null, 3, 5, 10].map(s => (
            ...
            <div className="space-y-4 text-center">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Bot Zorluğu</label>
            ...
            <div className="space-y-4 text-center">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Oyun Modları</label>
              <div className="flex justify-center gap-4">
                <button 
                  disabled={!!leagueState}
                ...
                >
                  {isGoalieMode ? 'KALECİLİ 🧤' : 'KALECİLİ'}
                </button>
                <button 
                  disabled={!!leagueState}
                ...
                >
                  {isFireMode ? 'ATEŞLİ 🔥' : 'ATEŞLİ'}
                </button>
              </div>
              {leagueState && <p className="text-[9px] text-slate-600 font-bold uppercase tracking-wider">Modlar ve Zorluk kariyer sırasında değiştirilemez</p>}
            </div>
            ...
            <div className="flex flex-col gap-4">
              {leagueState && (
                <button onClick={() => setView(GameView.DASHBOARD)} className="bg-blue-600 text-white py-5 rounded-2xl font-bold text-lg shadow-2xl flex items-center justify-center gap-3 hover:scale-[1.02] transition-transform border border-blue-400/50 group">
                  CONTINUE CAREER <span className="text-xl group-hover:translate-x-1 transition-transform">▶️</span>
                </button>
              )}
              <button onClick={() => setView(GameView.LEAGUE_SELECT)} className="bg-white text-slate-950 py-5 rounded-2xl font-bold text-lg shadow-2xl flex items-center justify-center gap-3 hover:scale-[1.02] transition-transform">START NEW CAREER <span className="text-blue-600">⚽</span></button>
              <button onClick={() => setView(GameView.MULTIPLAYER_SELECT)} className="bg-slate-800 text-white py-5 rounded-2xl font-bold text-lg flex items-center justify-center gap-3 hover:scale-[1.02] transition-transform">LOCAL 2P 🤜🤛</button>
            </div>
          </div>
        </div>
      )}
      ...
      {view === GameView.LEAGUE_SELECT && (
        <div className="w-full max-w-7xl h-full p-12 flex flex-col gap-10 animate-in slide-in-from-bottom">
          <div className="flex justify-between items-center">
             <h2 className="text-5xl font-sport font-black italic uppercase">Select <span className="text-blue-500">League</span></h2>
             <button onClick={() => setView(GameView.MENU)} className="bg-slate-800 px-6 py-2 rounded-xl font-bold">BACK</button>
          </div>
      ...
      {(view === GameView.TEAM_SELECT || view === GameView.MULTIPLAYER_SELECT) && (
        <div className="w-full max-w-7xl h-full p-12 flex flex-col gap-10">
          <div className="flex justify-between items-center">
            <h2 className="text-5xl font-sport font-black italic uppercase">{view === GameView.TEAM_SELECT ? "Choose Club" : "Local Match Selection"}</h2>
            <button onClick={() => setView(GameView.LEAGUE_SELECT)} className="bg-slate-800 px-6 py-2 rounded-xl font-bold">BACK</button>
          </div>
